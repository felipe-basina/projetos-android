package no.domain.boaviagem.repository;
 ...
 */
public class BoaViagemRepository {

    private DatabaseHelper helper;
    private SQLiteDatabase db;

    public BoaViagemRepository(Context context) {
    ...
    public List<Viagem> listarViagens() {
        Cursor cursor = this.getDb().query(DatabaseHelper.Viagem.TABELA,
                DatabaseHelper.Viagem.COLUNAS,
                null, null, null, null, null);
        List<Viagem> viagens = new ArrayList<Viagem>();
        while (cursor.moveToNext()) {
            Viagem viagem = this.criarViagem(cursor);
            viagens.add(viagem);
        }
        cursor.close();
        return viagens;
    ...
    public long inserir(Viagem viagem) {
        ...
        if (viagem.getId() == null) { // Insere registro
            return this.getDb().insert(DatabaseHelper.Viagem.TABELA,
                    null, values);
        } else { // Atualiza registro
            String whereClause = DatabaseHelper.Viagem._ID + " = ?";
            String[] whereArgs = new String[]{ viagem.getId().toString() };
            return this.getDb().update(DatabaseHelper.Viagem.TABELA, values, whereClause, whereArgs);
        }
    }
    ...
    public boolean removerViagem(Long id) {
        ...
        int removidos = this.getDb().delete(DatabaseHelper.Viagem.TABELA,
    ...
    public List<Gasto> listarGastos(Viagem viagem) {
        ...
        Cursor cursor = this.getDb().query(DatabaseHelper.Gasto.TABELA,
                DatabaseHelper.Gasto.COLUNAS,
                selection, selectionArgs,
                null, null, null);
        List<Gasto> gastos = new ArrayList<Gasto>();
        while (cursor.moveToNext()) {
            Gasto gasto = this.criarGasto(cursor);
            gastos.add(gasto);
        }
        cursor.close();
        return gastos;
    ...
    public long inserir(Gasto gasto) {
        ...
        if (gasto.getId() == null) {
            return this.getDb().insert(DatabaseHelper.Gasto.TABELA,
                    null, values);
        } else {
            String whereClause = DatabaseHelper.Gasto._ID + " = ?";
            String[] whereArgs = new String[]{ gasto.getId().toString() };
            return this.getDb().update(DatabaseHelper.Gasto.TABELA, values, whereClause, whereArgs);
        }
    }
    ...
    public boolean removerGasto(Long id) {
        String whereClause = DatabaseHelper.Gasto._ID + " = ?";
        String[] whereArgs = new String[]{id.toString()};
        int removidos = this.getDb().delete(DatabaseHelper.Gasto.TABELA,
                whereClause, whereArgs);
        return removidos > 0;
    }
